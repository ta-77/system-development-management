...
1. **ユーザー管理**
   - ユーザー登録・ログイン・ログアウト
   - パスワードのハッシュ化による安全な認証
   - **[更新箇所] 管理者権限の付与とアクセス制御**

2. **メニュー閲覧**
   - 学食メニューの一覧表示
   - カテゴリ別表示
   - 平均評価・レビュー数の確認
   - **[更新箇所] デザイン改善による視認性の向上**
...
4. **データ分析**
   - グラフによる可視化（人気ランキング、**[更新箇所] カテゴリ別平均、価格相関**）
   - データの傾向分析
...
2. **ログイン**
   - `/login` でログイン
   - 登録したユーザー名とパスワードを入力
   - 管理者機能を利用する場合は、配布された管理者アカウントで ` /login`
...
7. **[更新箇所]管理者操作**
   - 管理者アカウントでログイン時のみ表示される「管理者ページ」リンクをクリック
   - データの追加や削除を行う

## 画面遷移図[更新箇所]
   graph TD
    Login[ログイン画面] -->|ログイン成功| Index[メニュー一覧]
    Login -->|新規登録| Register[ユーザー登録画面]
    Register -->|登録完了| Login

    Index -->|詳細| Detail[メニュー詳細画面]
    Index -->|分析| Analysis[データ分析画面]
    Index -->|管理者のみ| Admin[管理者ページ]
    Index -->|ログアウト| Login

    Detail -->|投稿/編集| Post[レビュー投稿画面]
    Post -->|保存| Detail
    
    Admin -->|データ追加/削除| Admin
...
**[更新箇所]カテゴリー別の平均評価(棒グラフ)**
- 味・量・コスパ・総合の平均点をカテゴリごとに比較

**[更新箇所]価格と総合評価の関係**
- 価格帯ごとの満足度の分布をカテゴリ別に色分け表示
...
### 1. カテゴリー分析
- どのジャンル（定食、丼もの等）の満足度が高いかを多角的に可視化。

### 2. 価格分析
- 「価格が高い＝美味しい」のか、「安くて美味い」メニューはどこにあるのかを散布図で探索。
